...
public class OllamaVisionService {
    ...
    public OllamaVisionService(OllamaApi ollamaApi, OllamaChatModel chatModel, @Value("${OLLAMA_MODEL:}") String modelName) {
    ...
    public String compareImagesUsingCombining(String before, String after) throws IOException {
        ...
        var request = ChatRequest.builder(modelName).withStream(false) // not streaming
                .withMessages(List.of(Message.builder(Role.SYSTEM).withContent("You are a security monitor. You are looking for anything unusual.").build(),
                        Message.builder(Role.USER).withContent("The image contains two separate images, on the left and on the right. Compare these two images closely. What, if anything, is different between these two images?")
                                .withImages(List.of(base64Combined)).build())).withOptions(OllamaOptions.create().withTemperature(0.0)).build();
